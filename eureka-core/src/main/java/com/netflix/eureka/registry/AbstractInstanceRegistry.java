...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
    ...
    public void evict(long additionalLeaseMs) {
        ...
        for (Entry<String, Map<String, Lease<InstanceInfo>>> groupEntry : registry.entrySet()) {
            ...
            if (leaseMap != null) {
                for (Entry<String, Lease<InstanceInfo>> leaseEntry : leaseMap.entrySet()) {
                    ...
                    //lease.isExpired(additionalLeaseMs) 就是判断过期的核心
                    // 判断过期的逻辑： 当前时间 > 最后一次心跳时间 +  duration（默认90s） +  additionalLeaseMs（补偿时间）
                    //                最后一次心跳时间 = 发送心跳时间 + duration 这是Eureka的一个bug
                    //  最后得  当前时间 > 最后一次心跳时间 + 2* duration（默认90s） +  additionalLeaseMs（补偿时间）
        ...
        // 每次摘除不会一次将所有的过期实列， 会有一个比列，默认是0.85  默认最多移除0.15
    ...
    /* visible for testing */ class EvictionTask extends TimerTask {
        ...
        public void run() {
            try {
                // Compensation : 补偿金
                // 简单理解这就是一个补偿机制，这个定时器本来是60s执行一次，结果80s才执行一次
                // 那么这个compensationTimeMs 就会= 20s
